...
pub fn solve_part1(input: &str) -> usize {
    let bags: HashMap<String, HashSet<String>> =
...
fn calc(amount: usize, of: String, bags: &Bags) -> usize {
    match bags.get(&of) {
        Some(vec) => {
            let content: usize = vec
                .iter()
                .map(|(n, inside)| calc(*n, inside.to_string(), &bags))
                .sum();
...
pub fn solve_part2(input: &str) -> usize {
    let bags: Bags = input
        ...
        .flat_map(|l| {
            ...
            split
                .next()
                .and_then(|s| scan_bag(s))
                .map(|s| s.into_iter().collect::<HashSet<(usize, String)>>())
                .map(|s| (bag, s))
        })
        .collect();
