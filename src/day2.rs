...
pub fn input_generator(input: &str) -> Vec<Password> {
    input
        ...
        .map(|s| {
            ...
            // TODO String parsing
            let in_range = parts[0].split('-').map(|s| s.parse::<usize>().unwrap()).collect::<Vec<usize>>();
            // TODO Ranges
            Password {
...
fn validate(pass: &Password) -> bool {
    pass.range.contains(&pass.password.chars().filter(|c| c == &pass.char).count())
}
